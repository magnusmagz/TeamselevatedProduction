...
export default function SetParentPassword() {
  ...
  useEffect(() => {
    if (!token) {
      setError('Invalid invite link. Please ask your club admin to resend your invitation.');
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-login: store the returned JWT the same way Login does, then go to
      // the parent portal.
      if (data.token) {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white border-2 border-gray-200 p-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-brand-primary uppercase">SET UP YOUR PARENT ACCOUNT</h1>
            <p className="text-gray-600 mt-2">Choose a password to access the parent portal</p>
          </div>
