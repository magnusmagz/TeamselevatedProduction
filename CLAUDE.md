...
3. **Migration numbers**: 041–051 are taken, with COLLISIONS at 044 (payment_allocations /
   ...
   **064–072** applied since — with one that was NOT, and said it was: **069**
   ...
   pinned messages) are applied. **082** (`canva_assets`) and **084** (`programs_order_archive`, applied 2026-09-02 via `scripts/apply-migration.php`) are applied. **083** (`broadcast_campaign_body`), **085** (`program_staff`), **086** (`athlete_evaluations`), **087** (`tryout_coach_invites`) **088** (`field_size`) and **090** (`org_units`) applied 2026-09-02. **089** `scale_indexes`, **091** `compliance`, **092** `user_email_signature_format` and **093** `compliance_default_reminder_stream` applied 2026-09-03 (Heroku v592). Nothing written-but-unapplied as of that date. Next free number is **094**. **095** (`referee_feedback`, slice 8.6 / R68) is written on `feature/referee-feedback` and NOT applied — it is listed in both `PENDING_MIGRATION_TABLES` constants until the fixture refresh; `lib/referee_feedback.php` answers 503 until it runs. Apply migrations with `heroku run --no-tty -a teamselevated-backend php scripts/apply-migration.php NNN_name.sql` — it runs the file in one transaction and writes a `migration_applied` audit row, so CHANGELOG has something to cite.
